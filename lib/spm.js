...
(function() {
  ...
  StormPackageManager = (function(_super) {
        ...
        if (typeof modules.dependencies[entry].dependencies === 'object') {
          ...
          _results.push((function() {
            ...
            for (content in curobject) {
              ...
              if (firstime) {
                _results1.push(this.npmPackages[entry] = result.version);
              } else {
                if (this.npmPackages[entry] == null) {
                  this.emit("discovered", "npm", result);
                }
    ...
    StormPackageManager.prototype.execute = function(command, callback) {
      exec = require('child_process').exec;
      this.log('executing the command ', command);
    ...
    StormPackageManager.prototype.install = function(pinfo, ainclude, callback) {
      var cmd, err, filename, parsedurl, source, url, webreq;
      ...
      this.check(parsedurl.protocol, pinfo, (function(_this) {
        return function(pkg) {
      ...
      try {
        return this.execute(cmd, (function(_this) {
          return function(result) {
            var err, pkgconfig, plugfile, storm, _fn, _i, _len, _ref;
            if (result instanceof Error) {
              return callback(new Error(result));
            }
            if (parsedurl.protocol === "npm:") {
              try {
                pkgconfig = require("" + pinfo.name + "/package.json").config;
                storm = pkgconfig.storm;
                _this.log("install - [" + pinfo.name + "] processing storm compatible module...", pkgconfig, storm);
                if (storm.plugins == null) {
                  return callback(new Error("Not a storm compatible module"));
                }
                _this.log("install - [" + pinfo.name + "] available plugins:", storm.plugins);
                _ref = storm.plugins;
                _fn = function(plugfile) {
                  ...
                  _this.log("include - [" + pinfo.name + "] found valid plugin at " + plugfile);
                  return ainclude(plugin);
                };

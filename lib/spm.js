(function() {
  ...
  StormPackageManager = (function(_super) {
    ...
    _discoverNpmModules = function(callback) {
      var _this = this;
      return exec("npm ls --json --depth=0", function(error, stdout, stderr) {
    ...
    _discoverDebPkgs = function(callback) {
      var _this = this;
      return exec("dpkg -l | tail -n+6", function(error, stdout, stderr) {
        if (stdout) return callback(stdout);
      });
    };
    ...
    function StormPackageManager(context) {
      ...
      if (this.repeatInterval == null) this.repeatInterval = 8000;
      if (this.log == null) this.log = console.log;
    ...
    StormPackageManager.prototype.monitorDebPkgs = function(callback) {
      ...
      return _discoverDebPkgs(function(content) {
        if (content == null) return callback("success");
    ...
    StormPackageManager.prototype.analyzenpm = function(content, firstime) {
      var content, curobject, entry, modules, result, _base, _ref, _results;
      modules = JSON.parse(content);
      _results = [];
      for (entry in modules.dependencies) {
        result = {
          name: entry,
          version: (_ref = (_base = modules.dependencies[entry]).version) != null ? _ref : _base.version = "*",
          source: "builtin",
          type: "npm"
        };
        if (firstime) {
        ...
        if (typeof modules.dependencies[entry].dependencies === 'object') {
          ...
          _results.push((function() {
            var _base2, _ref2, _results2;
            _results2 = [];
            for (content in curobject) {
              result = {
                name: content,
                version: (_ref2 = (_base2 = curobject[content]).version) != null ? _ref2 : _base2.version = "*",
                source: 'dependency',
                type: "npm"
              };
              if (firstime) {
                _results2.push(this.npmPackages[result.name] = result.version);
              } else {
    ...
    StormPackageManager.prototype.monitorNpmModules = function(callback) {
      ...
      return _discoverNpmModules(function(stdout) {
        if (stdout == null) return callback("success");
    ...
    StormPackageManager.prototype.monitor = function(repeatInterval) {
      var _this = this;
      if (repeatInterval == null) repeatInterval = this.repeatInterval;
    ...
    StormPackageManager.prototype.install = function(pinfo, callback) {
      ...
      return this.check(parsedurl.protocol, pinfo, function(pkg) {
        var append, cmd, filename, source, webreq;
        ...
        switch (parsedurl.protocol) {
          ...
          case "dpkg:":
            ...
            webreq(source, function(error, response, body) {
              if (error != null) return new Error("unable to download file");
          ...
          case "apt-get:":
            pinfo.type = "dpkg";
            append = "";
            if (pinfo.version !== "*") append = "=" + pinfo.version;

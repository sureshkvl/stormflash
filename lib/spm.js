...
(function() {
  ...
  StormPackageManager = (function(superClass) {
    ...
    function StormPackageManager(context) {
      ...
      this.syncInstallerQueue = async.queue((function(_this) {
        return function(task, callback) {
          _this.log('running installer queue ', task);
          return _this.execute(task.cmd, function(result) {
            if (result instanceof Error) {
              return callback(new Error);
            }
            return callback(task.pinfo);
          });
        };
      })(this), 1);
    ...
    StormPackageManager.prototype.analyzeDeb = function(content, firstime) {
      var contents, i, len, pkg, result, results;
      if (content != null) {
        ...
        for (i = 0, len = contents.length; i < len; i++) {
          ...
          if ((content[1] != null) && (content[2] != null)) {
            ...
            if (firstime) {
              results.push(this.debPackages[result.name] = result.version);
            } else {
    ...
    StormPackageManager.prototype.analyzenpm = function(content, firstime) {
      var base, curobject, entry, modules, result, results;
      modules = JSON.parse(content);
      results = [];
      for (entry in modules.dependencies) {
        result = {
          name: entry,
          version: (base = modules.dependencies[entry]).version != null ? base.version : base.version = "*",
          source: "builtin",
          type: "npm"
        };
        if (firstime) {
        ...
        if (typeof modules.dependencies[entry].dependencies === 'object') {
          ...
          results.push((function() {
            ...
            for (content in curobject) {
              result = {
                name: content,
                version: (base1 = curobject[content]).version != null ? base1.version : base1.version = "*",
                source: 'dependency',
                type: "npm"
              };
              if (firstime) {
                results1.push(this.npmPackages[result.name] = result.version);
              } else {
    ...
    StormPackageManager.prototype.getCommand = function(installer, command, component, filename) {
      ...
      switch (installer + "." + command) {
        ...
        case "npm.install":
          if (!filename) {
            return "npm install -g --production --loglevel error " + component.name + "@" + component.version;
          }
    ...
    StormPackageManager.prototype.install = function(pinfo, callback) {
      ...
      return this.check(parsedurl.protocol, pinfo, (function(_this) {
        return function(pkg) {
          var append, cmd, err, filename, source, task, webreq;
          ...
          switch (parsedurl.protocol) {
            ...
            case "dpkg:":
              ...
              webreq(source, function(error, response, body) {
                ...
                if (fs.existsSync(filename)) {
                  ...
                  task.cmd = cmd;
                  task.pinfo = pinfo;
                  return _this.syncInstallerQueue.push(task, callback);
                } else {
            ...
            case "apt-get:":
              ...
              task.cmd = cmd;
              task.pinfo = pinfo;
              _this.syncInstallerQueue.push(task, callback);
              return;
            default:

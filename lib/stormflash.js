(function() {
  var StormBolt, StormData, StormFlash, StormInstance, StormInstances, StormPackage, StormPackages, StormRegistry, agent, config, query, storm,
  ...
  StormPackages = (function(_super) {
    ...
    StormPackages.prototype.match = function(pinfo) {
      ...
      if (packages[0] != null) this.log("Matched Package ", packages[0]);
      return packages[0];
    };
    ...
    StormPackages.prototype.find = function(name, version) {
      ...
      if (packages[0] != null) this.log("Found Package ", packages[0]);
      return packages[0];
    };
  ...
  StormFlash = (function(_super) {
    ...
    function StormFlash(config) {
      ...
      this.instances.on('ready', function() {
    ...
    StormFlash.prototype.run = function(config) {
      ...
      this.spm.on('discovered', function(pkgType, pinfo) {
        var pkg, spkg;
      ...
      this.packages.on('added', function(pkginfo) {
        var pkg;
        if (!((pkginfo != null) || (pkginfo.data != null))) return;
        pkg = pkginfo.data;
        if (pkg.source === "builtin" || pkg.source === "dependency") return;
        if (pkg.type === "npm") return _this["import"](pkg.name);
      });
      this.packages.on('ready', function() {
          ...
          case "exited":
            ...
            if ((entry != null) && entry.monitorOn === true) {
              ...
              return _this.start(key, function(key, pid) {
                if (key instanceof Error) _this.log(key);
              });
            }
            break;
          case "error":
      ...
      this.processmgr.on("stopped", function(signal, pid, key) {
        var entry;
        if (signal != null) _this.log("process stopped due to signal ", signal);
        if (key != null) entry = _this.instances.entries[key];
        if (entry != null) {
          if (entry != null) {
      ...
      this.instances.on("attachnMonitor", function(pid, key) {
      ...
      return this.processmgr.on("monitor", function(pid, key) {
    ...
    StormFlash.prototype.install = function(pinfo, callback) {
      ...
      return this.spm.install(pinfo, function(pkg) {
        var result;
        if (pkg instanceof Error) return callback(new Error(pkg));
        spkg.data = pkg;
        result = _this.packages.add(spkg.id, spkg);
        result.data.id = result.id;
        _this.emit('installed the package ', result);
    ...
    StormFlash.prototype.uninstall = function(pinfo, callback) {
      ...
      return this.spm.uninstall(pinfo, function(result) {
        if (result instanceof Error) return callback(new Error(result));
    ...
    StormFlash.prototype.start = function(key, callback) {
      ...
      if (pid == null) return callback(new Error("Not able to start the binary"));
      entry.data.pid = pid;
      if (entry.data.monitor === true) entry.monitorOn = true;
      entry.saved = false;
      this.instances.update(key, entry);
      this.processmgr.attach(pid, key);
      if (callback != null) callback(key, pid);
    ...
    StormFlash.prototype.restart = function(key, callback) {
      ...
      if (!(status instanceof Error)) {
        ...
        entry.saved = false;
        this.instances.update(key, entry);
        if (callback != null) return callback(key, pid);
      }
    };

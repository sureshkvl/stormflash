...
(function() {
  ...
  StormInstances = (function(_super) {
    ...
    StormInstances.prototype.discover = function() {
      ...
      this.log("Dumping all entries", this.entries);
      _results = [];
  ...
  StormFlash = (function(_super) {
    ...
    function StormFlash(config) {
      ...
      this.log("Waiting for event ready");
      this.instances.on('ready', (function(_this) {
        return function() {
          _this.log("Ready event");
    ...
    StormFlash.prototype.run = function(config) {
      ...
      this.processmgr.on("signal", (function(_this) {
        return function(signal, signum, pid, key) {
          var entry;
          _this.log("recieved signal " + signal + " with number - " + signum + " from pid " + pid + " with key " + key);
          switch (signal) {
            case "stopped":
            case "killed":
            case "exited":
              ...
              if ((entry != null) && entry.monitorOn === true) {
                ...
                entry.monitorOn = false;
                _this.processmgr.stop(signum, pid, key);
                return async.series([
                  function(startProc) {
                    return setTimeout(startProc, 1000);
                  }
                ], function() {
    ...
    StormFlash.prototype.uninstall = function(pinfo, callback) {
      ...
      if (instance != null) {
        this.processmgr.once('stop', function(key, pid) {
          this.processmgr.stop(9, pid, key);
    ...
    StormFlash.prototype.stop = function(key, callback) {
      ...
      entry.saved = false;
      this.instances.update(key, entry);
      return this.processmgr.stop(9, entry.data.pid, key);
    };
    ...
    StormFlash.prototype.restart = function(key, callback) {
      ...
      entry = this.instances.entries[key];
      entry.monitorOn = false;
      status = this.processmgr.stop(9, entry.data.pid, key);

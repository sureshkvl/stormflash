  ...
  StormFlash = (function(_super) {
    ...
    function StormFlash(config) {
      ...
      this.spm.on('discovered', function(pkgType, pinfo) {
        ...
        if (pkg == null) {
          if (pinfo.source == null) pinfo.source = "builtin";
    ...
    StormFlash.prototype.install = function(pinfo, callback) {
      var pkg,
        _this = this;
      console.log(pinfo);
      this.log("checking for package " + pinfo.data.name + " in DB", pinfo);
      pkg = this.packages.match(pinfo.data);
      console.log("pkg is ", pkg);
      if (pkg != null) {
        this.log("Found matching package name " + pkg.name);
        callback(pkg);
      }
      return this.spm.install(pinfo.data, function(pkg) {
        if (pkg instanceof Error) return callback(pkg);
    ...
    StormFlash.prototype.uninstall = function(pinfo, callback) {
      ...
      if (pkg instanceof Error) return 404;
      ...
      return this.spm.uninstall(pinfo, function(result) {
        if (result instanceof Error) return callback(500);
    ...
    StormFlash.prototype.start = function(key, callback) {
      ...
      if (pid == null) callback(new Error("Not able to start the binary"));
      entry.data.pid = pid;

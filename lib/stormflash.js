...
(function() {
  ...
  StormFlash = (function(_super) {
    ...
    StormFlash.prototype.invoke = function(service, callback) {
      ...
      service.once('destroy', (function(_this) {
        return function() {
          _this.log("service.destroy called for " + service.id + " invoked with:", service.invocation);
          return _this.processmgr.stop(service.instance, service.id);
        };
      })(this));
      service.on('change', (function(_this) {
        return function() {
          var waitCounter;
          _this.log("service.change called for " + service.id + " invoked with:", service.invocation);
          ...
          return async.until(function() {
            ...
            _this.log(("" + service.id + " has successfully stopped, took ") + (waitCounter * 100 / 1000) + " seconds");
            opts = service.invocation;
            ...
            return service.emit('running', pid);
          });
        };
      })(this));
      if (opts.monitor) {
        this.processmgr.attach(pid, service.id);
        this.processmgr.monitor(pid, service.id);
      }
    ...
    StormFlash.prototype.start = function(key, callback) {
      ...
      this.log("Environment options to start the process : ", entry.data.options);
      ...
      if (!(status instanceof Error)) {
        return async.series([
          ...
          return function() {
            ...
            _this.log("Environment options to restart the process : ", entry.data.options);

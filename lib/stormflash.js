...
(function() {
  ...
  StormInstances = (function(_super) {
    ...
    StormInstances.prototype.get = function(key) {
      ...
      if ((entry.data != null) && entry.data instanceof StormInstance) {
  ...
  StormPackages = (function(_super) {
    ...
    StormPackages.prototype.match = function(pinfo) {
      ...
      for (key in this.entries) {
        ...
        pkg = entry.data;
        if ((pkg.name === pinfo.name) && ((pkg.version === pinfo.version) || (pinfo.version === "*")) && ((pkg.source === pinfo.source) || (pkg.source === "builtin") || (pkg.source === "dependency"))) {
          pkg.id = entry.id;
          return pkg;
        }
      }
    ...
    StormPackages.prototype.find = function(name, version) {
      ...
      for (key in this.entries) {
        ...
        pkg = entry.data;
        if ((pkg.name === name) && ((pkg.version === version) || (pkg.version === "*"))) {
          this.log("find - Matching found for " + pkg.name + " and " + pkg.version);
          entry.data.id = entry.id;
          return entry.data;
        }
      }
      ...
      this.spm.on('discovered', (function(_this) {
        return function(pkgType, pinfo) {
          ...
          if (pkg == null) {
            ...
            return _this.packages.add(spkg.id, spkg.data);
          } else {
            if (pkgType === "npm") {
              _this.log("test: discovered package that existed in Db " + pkg.name + " is having source ", pkg.source);
              if (pkg.source === "builtin" || pkg.source === "dependency") {
                return;
              }
              return _this["import"](pinfo.name);
            }
          }
        };
      })(this));
      this.spm.monitor(this.config.repeatInterval);
    ...
    StormFlash.prototype.install = function(pinfo, callback) {
      ...
      return this.spm.install(pinfo, (function(_this) {
        return function(pkg) {
          ...
          spkg = new StormPackage(null, pinfo);
          result = _this.packages.add(spkg.id, spkg.data);

...
(function() {
  ...
  StormInstance = (function(superClass) {
    var schema;

    extend(StormInstance, superClass);
  ...
  StormInstances = (function(superClass) {
    extend(StormInstances, superClass);
      ...
      for (key in this.entries) {
        ...
        if ((entry != null) && (entry.data != null) && (entry.data.pid != null)) {
          if (entry.data.monitor === true) {
            ...
            results.push(this.emit("attachnMonitor", entry.data.pid, key));
          } else {
            results.push(void 0);
          }
        } else {
  ...
  StormPackage = (function(superClass) {
    var schema;

    extend(StormPackage, superClass);
  ...
  StormPackages = (function(superClass) {
    extend(StormPackages, superClass);
    ...
    function StormPackages(filename) {
      this.on('load', function(key, val) {
        ...
        if (entry != null) {
          entry.saved = true;
          this.add(key, entry);
          if (entry.data.type === "npm" && /-storm/i.test(entry.data.name)) {
  ...
  StormFlash = (function(superClass) {
    var exec, fs, path, processmgr, spm, uuid, validate;

    extend(StormFlash, superClass);
    ...
    function StormFlash(config) {
      ...
      fs.mkdir(this.config.datadir + "/meta", function() {});
    ...
    StormFlash.prototype.run = function(config) {
      ...
      this.spm.on('discovered', (function(_this) {
        return function(pkgType, pinfo) {
          ...
          if (pkg == null) {
            ...
            _this.packages.add(spkg.id, spkg);
            if (spkg.data.type === "npm") {
              spkg.data.status = {};
      ...
      this.packageQueue = async.queue((function(_this) {
        return function(task, callback) {
          return _this.spm.install(task.data, function(pkg) {
            var base, result;
            if (pkg instanceof Error) {
              return typeof callback === "function" ? callback(new Error(pkg)) : void 0;
            }
            task.data = pkg;
            if ((base = task.data).status == null) {
              base.status = {};
            }
            task.data.status.installed = true;
            task.data.status.imported = false;
            result = _this.packages.update(task.id, task);
      ...
      this.packages.on('updated', (function(_this) {
        return function(pkginfo) {
          var err, pkg;
          _this.log("Update event");
          if (!((pkginfo != null) || (pkginfo.data != null))) {
            return;
          }
          pkg = pkginfo.data;
          if (pkg.type === "npm" && /-storm/i.test(pkg.name)) {
            try {
              _this["import"](pkg.name);
            } catch (_error) {
              err = _error;
              _this.log("Not able to import the module " + pkg.name);
            }
    ...
    StormFlash.prototype.install = function(pinfo, callback) {
      ...
      result.data.id = result.id;
      callback(result);
      return this.packageQueue.push(spkg);
    };
